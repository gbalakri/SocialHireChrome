...
(function () {
  ...
  let panelOpen = false;
  let pageType  = LSFScrape.detectPageType();

  // ── Helpers ──────────────────────────────────────────────────────────────
  // Pure DOM-scraping helpers live in scrape.js (loaded first) as `LSFScrape`.
  ...
  function prefill() {
    if (pageType === 'person') {
      const d = LSFScrape.scrapeProfile();
      ...
      const d = LSFScrape.scrapeCompany();
  ...
  const navObserver = new MutationObserver(() => {
    if (location.href !== lastURL) {
      ...
      const newType = LSFScrape.detectPageType();
      if (newType !== pageType) {
        pageType = newType;
